...
- 🔧 **`nodeVersionMatchesDdev()`** - Validates the project pins a Node version in both `package.json` `engines.node` and `.nvmrc` (compatible with each other) and that `.ddev/config.yaml` sets `nodejs_version: auto` so DDEV derives the Node version from the project *(creates the missing constraint — defaulting to Node 24 when none is declared — and sets `nodejs_version: auto`; a conflict between existing `engines.node` and `.nvmrc` is reported, not auto-resolved)*
