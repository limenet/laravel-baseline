...
- 🔧 **`doesNotHaveCopilotOrJunieAgentFiles()`** - Fails if `AGENTS.md` or a `.junie` directory exist — these are generated for the Copilot/Junie Boost agents, which are no longer required *(deletes `AGENTS.md` and the `.junie` directory)*
