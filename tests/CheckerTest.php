...
it('phpstanLevelAtLeastEight fails when phpstan.neon is missing', function (): void {
    ...
    $checker = new Checker(makeCommand());
    expect($checker->phpstanLevelAtLeastEight())->toBe(CheckResult::FAIL);
    expect($checker->getComments())->toContain('PHPStan configuration missing: Create phpstan.neon in project root');
});
...
YAML;
    ...
    $checker = new Checker(makeCommand());
    expect($checker->phpstanLevelAtLeastEight())->toBe(CheckResult::FAIL);
    expect($checker->getComments())->toContain('PHPStan level not configured: Add "level" parameter to phpstan.neon');
});
...
YAML;
    ...
    $checker = new Checker(makeCommand());
    expect($checker->phpstanLevelAtLeastEight())->toBe(CheckResult::FAIL);
    expect($checker->getComments())->toContain('PHPStan level must be at least 8: Found level 5 in phpstan.neon (set to 8 or higher)');
});
...
YAML;
    ...
    $checker = new Checker(makeCommand());
    expect($checker->phpstanLevelAtLeastEight())->toBe(CheckResult::PASS);
});
...
YAML;
    ...
    $checker = new Checker(makeCommand());
    expect($checker->phpstanLevelAtLeastEight())->toBe(CheckResult::FAIL);
    expect($checker->getComments())->toContain('PHPStan level must be a number or "max": Found "invalid" in phpstan.neon');
});
...
it('hasClaudeSettingsWithLaravelSimplifier passes when settings file has correct configuration', function (): void {
    ...
    expect((new Checker(makeCommand()))->hasClaudeSettingsWithLaravelSimplifier())->toBe(CheckResult::PASS);
});
...
it('hasClaudeSettingsWithLaravelSimplifier fails when settings file is missing', function (): void {
    ...
    $checker = new Checker(makeCommand());
    $result = $checker->hasClaudeSettingsWithLaravelSimplifier();
